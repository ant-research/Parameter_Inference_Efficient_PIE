...
class KGDataset:
    ...
    * test stores the triples in the test set.
    ...
    def __init__(self, entity_path, relation_path, train_path,
                 valid_path=None, test_path=None, e2r_path=None, format=[0, 1, 2],
        ...
        if test_path is not None:
            ...
            self.test = None
        if e2r_path is not None:
            self.e2r = np.load(e2r_path)
            #self.e2r = scipy.sparse.load_npz(e2r_path)
        else:
            self.e2r = None
    ...
    * test.txt stores the triples in the test set.
    ...
    def __init__(self, path, name='FB15k'):
        ...
        super(KGDatasetFB15k, self).__init__(os.path.join(self.path, 'entities.dict'),
                                             ...
                                             os.path.join(self.path, 'e2r_scores.npy')
                                             )
...
class KGDatasetWiki(KGDataset):
    ...
    def __init__(self, path, name='wikikg90m', test_mode='test-dev'):
        ...
        self.valid = None
        self.test = None
        self.valid_dict = self.dataset.valid_dict(True)
        self.test_dict = self.dataset.test_dict(test_mode, True)
        self.entity_feat = self.dataset.entity_feat
        self.relation_feat = self.dataset.relation_feat
        ...
        if 't,r->h' in self.test_dict:
            del self.valid_dict['t,r->h']
        if os.path.exists(os.path.join(path, 'wikikg90m-v2/processed/e2r_scores.npz')):
            self.e2r = scipy.sparse.load_npz(os.path.join(path, 'wikikg90m-v2/processed/e2r_scores.npz'))
        else:
            self.e2r = None     
