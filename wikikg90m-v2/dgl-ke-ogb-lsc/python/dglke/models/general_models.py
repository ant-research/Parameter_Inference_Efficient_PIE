        ...
        if triplet_wise:
            ...
            for i in range((num_head + batch_size - 1) // batch_size):
                ...
                for j in range((num_tail + batch_size - 1) // batch_size):
                    ...
                    s_score.append(F.copy_to(self.score_func.infer(
                        sh_emb, rel_emb, st_emb), F.cpu()))
...
class KEModel(object):
    ...
    def share_memory(self):
        ...
        """
        if self.encoder_model_name in ['concat', 'shallow', 'concat_v1']:
            if self.LRE:
                self.entity_emb_base.share_memory()
                self.entity_emb_index.share_memory()
            else:
                self.entity_emb.share_memory()
        if self.encoder_model_name in ['concat', 'roberta', 'concat_v1']:
